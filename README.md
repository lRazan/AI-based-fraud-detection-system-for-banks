...
When a customer receives a fraud alert email, they can click YES or NO to confirm the transaction.  
This response is sent to a **Flask-based API** deployed on **Render**, which then updates the feedback in the database.

- 🔗 Confirmation URL format:  
  `https://flask-confirm-api.onrender.com/confirm?tx=<transaction_id>&r=YES`

- ✅ Response options: `YES` or `NO`  
- 🗃️ Storage: The response is inserted into a `transaction_feedback` table in the same Railway-hosted MySQL database  
...
- `Streamlit Cloud`, `Railway` — App and database deployment  
...
- **Streamlit Cloud** — hosting the web interface  
- **Railway** — hosting the MySQL database  
- **Render** — hosting the Flask API that handles customer confirmations  
